...
namespace JLChnToZ.NDExtensions.Editors {
    public sealed class ParameterCache {
        ...
        readonly Dictionary<(string, Component), AnimatorControllerParameterType> parameters = new();
        readonly Type[] ignoreComponents;

        public IReadOnlyDictionary<(string propertyName, Component source), AnimatorControllerParameterType> Parameters => parameters;
        ...
        public void Refresh() {
            ...
            foreach (var p in info) {
                if (p.ParameterType == null ||
                    ...
                    continue;
                parameters[(p.OriginalName, p.Source)] = p.ParameterType.Value;
                if (p.Source != null &&
